...
bool Estabelecimento::reabastecer(std::string nome, int quant)
{
    for (size_t i=0; i<fornecedor.produtos.size(); ++i)
    {
        if (fornecedor.produtos[i].descricao == nome && fornecedor.produtos[i].quantidade > 0)
        {
            if (fornecedor.repassar_produto(nome, quant))
            {
                for (size_t j=0; j<produtos.size(); ++j)
                {
                    if (produtos[j].descricao == nome) 
                    {
                        produtos[j].quantidade += quant;
                        return true;
                    }
                }
            }
